"""
Safe Nightly Improvement — replaces the dangerous code-rewriting experiment.

Instead of modifying Bob's source code, the nightly improvement cycle:
1. Reviews performance (retro) — what worked, what failed
...
This is the orchestrator of the OpenClaw-style safe self-improvement loop:
  Skills + Config Tuning + Plugins + Safe Nightly Improvement (here)
"""
...
logger = logging.getLogger("mind_clone.services.safe_improve")
...
def run_safe_improvement(owner_id: int) -> dict[str, Any]:
    ...
    if failures:
        report_lines.append("⚠️ *Failure Patterns:*")
        for f in failures:
            report_lines.append(
    ...
    if isinstance(tune_changes, list) and tune_changes:
        report_lines.append("")
        report_lines.append("⚙️ *Config Tuning:*")
        ...
        report_lines.append("⚙️ *Config:* No tuning needed")
...
# ---------------------------------------------------------------------------
# Tool wrapper (called from the tool registry)
# ---------------------------------------------------------------------------
